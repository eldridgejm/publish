...
ArtifactLocation = collections.namedtuple('ArtifactLocation', [
    'artifact_key',
    'artifact',
    'publication_key',
    'publication',
    ...
    for collection_key, collection in universe.collections.items():
        for publication_key, publication in collection.publications.items():
            for artifact_key, artifact in publication.artifacts.items():
                yield ArtifactLocation(artifact_key, artifact, publication_key, publication, collection_key, collection)
...
def release_calendar(cwd, date_context, show_published=False, skip_directories=None):
    universe = discover(cwd, date_context=date_context, skip_directories=skip_directories)
    ...
    for loc, time in sorted_times:
        if time > now or show_published:
            ...
            print('\t', loc.collection_key, '/', loc.publication_key, '/', loc.artifact_key)


def cli():
    parser = argparse.ArgumentParser()
    parser.add_argument('path', default=pathlib.Path.cwd(), nargs='?', type=pathlib.Path)
    parser.add_argument(
        "--start-of-week-one",
    ...
    release_calendar(args.path, date_context, skip_directories=args.skip_directories,
